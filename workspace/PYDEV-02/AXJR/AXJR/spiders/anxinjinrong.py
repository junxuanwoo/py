...
class AXJR_Spider(Spider):
    ...
    start_urls = common.getUrls('http://www.anxinjr.com/listall?page=', 1, '', '正在还款', 0)
    ...
    def parse_axjr(self, response):
        sel = Selector(response)
        items, values = [], ()
        item = ProductItem()

        item['platformid'] = 'P00522'
        item['productname'] = sel.xpath('//div[@class="infoList2 clearfx"]/div/h2/text()')[0].extract()#产品名称
        producttype = ''.join(re.findall(ur"[\u4e00-\u9fa5]+", item['productname']))
        item['producttype'] = producttype.replace(ur'第', '')
