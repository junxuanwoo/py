...
class RBW_Spider(Spider):
    # 融贝网
    name = "RBW"
    allowed_domains = ["www.irongbei.com"]
    conn = DBConn.get_instance()
    common = Common.get_instance()
    regex = re.compile(r'\d+')
    start_urls = common.getUrls('https://www.irongbei.com/index/plist?page=', 1, '', '立即购买')
    ...
    def parse_rbw(self, response):
        ...
        item['amount'] = int(float(infos[0])*10000)# 总额
        item['maxrate'] = item['minrate'] = infos[1]
