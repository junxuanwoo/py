...
from MDTX.items import MDTX_product
...
class MDTX_Spider(Spider):
    ...
    urls = getUrls('http://www.mindai.com/grtzlc?app_type=youxuan&ui_type=inline&order=default&currentPage=', 1, '', '立即抢购')
    print "urls = ", urls

    def parse(self, response):
        sel = Selector(response)
        homeUrl = 'https://www.mindai.com/'
        #product=sel.xpath('//div[@class="header_l"]/span[@class="hot_line"]/text()')[0].extract()
        #product_urls  = sel.xpath('//div[@class="bid_bu"]/a[@href]').extract()
        #获取当前列表页中所有在售新标的url跳转链接
        product_urls  = sel.xpath('//td[@class="td_first sign"]/a[@href]').extract()
        for product_url in product_urls:
            newUrl = homeUrl.join(product_url)
            print "newUrl = ", newUrl
